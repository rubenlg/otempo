    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager,
        ...
        if (!firstStation(context)) {
            ...
            for (int i=0; i<N; i++) {
                ...
                PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, otempoIntent, 0);
    ...
    public static void updateStation(Context context, Station station) {
        ...
        if (prediction == null) {
            ...
            if (prediction instanceof StationShortTermPrediction) {
                ...
                rviews.setTextViewText(R.id.widgetTemps, pred.getMinTemp()+" - "+pred.getMaxTemp() + " C");
                rviews.setImageViewResource(R.id.widgetMorning, ResourceUtils.getResource(pred.getSkyStateMorning(), true));
                rviews.setImageViewResource(R.id.widgetAfternoon, ResourceUtils.getResource(pred.getSkyStateAfternoon(), true));
                rviews.setImageViewResource(R.id.widgetEvening, ResourceUtils.getResource(pred.getSkyStateNight(), false));
                ...
                rviews = new RemoteViews(context.getPackageName(),  R.layout.widget_layout_medium_term);
                rviews.setImageViewResource(R.id.widgetMorning, ResourceUtils.getResource(predMed.getSkyState(), true));
            } else {
                rviews = new RemoteViews(context.getPackageName(),  R.layout.widget_layout_err);
